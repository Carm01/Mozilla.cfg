...
#AutoIt3Wrapper_Icon=Papirus-Team-Papirus-Apps-Firefox-trunk-alt.ico
#AutoIt3Wrapper_Outfile=FirefoxUpdater_x64Developer_x.exe
#AutoIt3Wrapper_Compression=4
#AutoIt3Wrapper_UseX64=y
#AutoIt3Wrapper_Res_Comment=Preformes a clean install of Firefox 64bit Nightly version. Auto downloads latest version.
#AutoIt3Wrapper_Res_Description=Updates to the Latest 64bit Nightly
...
Func killapps()
	...
	If FileExists("C:\Program Files\Firefox Developer Edition\") Then
	...
	If FileExists("C:\Program Files (x86)\Firefox Developer Edition\") Then
		$cmd = "taskkill.exe /im firefox.exe /f /t"
		RunWait('"' & @ComSpec & '" /c ' & $cmd, @SystemDir, @SW_HIDE)
		Sleep(300)
		$cmd = "taskkill.exe /im plugin* /f /t"
...
Func remcfg64()
	...
	$SSlist = _FileListToArrayRec("C:\Program Files\Firefox Developer Edition\", "*", $FLTAR_FILES, $FLTAR_RECUR, $FLTAR_NOSORT, $FLTAR_FULLPATH)
...
Func remcfg32()
	$SSlist = _FileListToArrayRec("C:\Program Files (x86)\Firefox Developer Edition\", "*", $FLTAR_FILES, $FLTAR_RECUR, $FLTAR_NOSORT, $FLTAR_FULLPATH)
	For $i = 1 To UBound($SSlist) - 1
		If StringInStr($SSlist[$i], "mozilla.cfg") > 1 Then
			FileDelete($SSlist[$i])
		ElseIf StringInStr($SSlist[$i], "override.ini") > 1 Then
...
Func uninstall()
	...
	If FileExists("C:\Program Files\Firefox Developer Edition\Firefox.exe") Then
		ShellExecuteWait("helper.exe", " /S", "C:\Program Files\Firefox Developer Edition\uninstall")
	EndIf

	If FileExists("C:\Program Files (x86)\Firefox Developer Edition\Firefox.exe") Then
		ShellExecuteWait("helper.exe", " /S", "C:\Program Files (x86)\Firefox Developer Edition\uninstall")
	EndIf

	Sleep(1500)
	DirRemove("C:\Program Files (x86)\Firefox Developer Edition\", 1)
	DirRemove("C:\Program Files\Firefox Developer Edition\", 1)
EndFunc   ;==>uninstall
...
Func getfiles()
	...
	$sSite = "https://www.mozilla.org/en-US/firefox/developer/all/"
	;$sNotes = "https://www.mozilla.org/en-US/firefox/beta/notes/"
	$sNotes = "https://www.mozilla.org/en-US/firefox/developer/notes/"
...
Func dlinstall()
	...
	If FileExists(@ScriptDir & "\mozilla.cfg") Then
		FileCopy(@ScriptDir & "\mozilla.cfg", "C:\Program Files\Firefox Developer Edition\mozilla.cfg", 1)
	Else
		FileInstall("mozilla.cfg", "C:\Program Files\Firefox Developer Edition\mozilla.cfg", 1)
	EndIf

	FileWrite("C:\Program Files\Firefox Developer Edition\defaults\pref\local-settings.js", 'pref("general.config.obscure_value", 0);' & @CRLF & 'pref("general.config.filename", "mozilla.cfg");' & @CRLF)
	FileWrite("C:\Program Files\Firefox Developer Edition\browser\override.ini", '[XRE]' & @CRLF & 'EnableProfileMigrator=false' & @CRLF)
	FileDelete("C:\windows\temp\" & "\FFInstall.exe")
	$CVersion = FileGetVersion('C:\Program Files\Firefox Developer Edition\firefox.exe', $FV_PRODUCTVERSION)
	SplashOff()
EndFunc   ;==>dlinstall

Func _webDownloader($sSourceURL, $sTargetName, $sVisibleName, $sTargetDir = "C:\windows\temp", $bProgressOff = True, $iEndMsgTime = 2000, $sDownloaderTitle = "Firefox Nightly")
...
Func EventLog()
	...
	If $ecode = '404' Then
		...
		_EventLog__Report($hEventLog, 1, 0, 404, @UserName, @UserName & ' No "exe" found for Firefox Developer Edition. The webpage and/or download link might have changed. ' & @CRLF, $aData)
	...
	If $ecode = '411' Then
		...
		$hEventLog = _EventLog__Open("", "Application")
		_EventLog__Report($hEventLog, 0, 0, 411, @UserName, @UserName & " Firefox Developer Edition " & "version " & $CVersion & " successfully installed." & @CRLF, $aData)
		_EventLog__Close($hEventLog)
	EndIf
	...
	If $ecode = '444' Then
		...
		$hEventLog = _EventLog__Open("", "Application")
		_EventLog__Report($hEventLog, 0, 0, 444, @UserName, @UserName & " The current version of Firefox Developer Edition is already installed " & $CVersion & @CRLF, $aData)
		_EventLog__Close($hEventLog)
	EndIf
EndFunc   ;==>EventLog
...
Func About()
	...
	MsgBox($MB_SYSTEMMODAL, "", "Firefox Developer Edition Update Tool" & @CRLF & @CRLF & _
			"Version: 2.5.0.0" & @CRLF & _
			"Firefox Developer Edition Updater by Carm0@Sourceforge" & @CRLF & "CTRL+ALT+m to kill", 5) ; Find the folder of a full path.
EndFunc   ;==>About
