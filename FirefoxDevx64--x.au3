...
#AutoIt3Wrapper_Outfile=FirefoxUpdater(x64Nightly)_x.exe
...
Func killapps()
	...
	If FileExists("C:\Program Files\Firefox Nightly\") Then
	...
	If FileExists("C:\Program Files (x86)\Firefox Nightly\") Then
		$cmd = "taskkill.exe /im firefox.exe /f /t"
		RunWait('"' & @ComSpec & '" /c ' & $cmd, @SystemDir, @SW_HIDE)
		Sleep(300)
		$cmd = "taskkill.exe /im plugin* /f /t"
...
Func remcfg64()
	...
	$SSlist = _FileListToArrayRec("C:\Program Files\Firefox Nightly\", "*", $FLTAR_FILES, $FLTAR_RECUR, $FLTAR_NOSORT, $FLTAR_FULLPATH)
...
Func remcfg32()
	$SSlist = _FileListToArrayRec("C:\Program Files (x86)\Firefox Nightly\", "*", $FLTAR_FILES, $FLTAR_RECUR, $FLTAR_NOSORT, $FLTAR_FULLPATH)
	For $i = 1 To UBound($SSlist) - 1
		If StringInStr($SSlist[$i], "mozilla.cfg") > 1 Then
			FileDelete($SSlist[$i])
		ElseIf StringInStr($SSlist[$i], "override.ini") > 1 Then
...
Func uninstall()
	...
	If FileExists("C:\Program Files\Firefox Nightly\Firefox.exe") Then
		ShellExecuteWait("helper.exe", " /S", "C:\Program Files\Firefox Nightly\uninstall")
	EndIf

	If FileExists("C:\Program Files (x86)\Firefox Nightly\Firefox.exe") Then
		ShellExecuteWait("helper.exe", " /S", "C:\Program Files (x86)\Firefox Nightly\uninstall")
	EndIf

	Sleep(1500)
	DirRemove("C:\Program Files (x86)\Firefox Nightly\", 1)
	DirRemove("C:\Program Files\Firefox Nightly\", 1)
EndFunc   ;==>uninstall
...
Func getfiles()
	...
	$sSite = "https://www.mozilla.org/en-US/firefox/nightly/all/"
	;$sNotes = "https://www.mozilla.org/en-US/firefox/beta/notes/"
	$sNotes = "https://www.mozilla.org/en-US/firefox/nightly/notes/"
...
Func dlinstall()
	...
	If FileExists(@ScriptDir & "\mozilla.cfg") Then
		FileCopy(@ScriptDir & "\mozilla.cfg", "C:\Program Files\Firefox Nightly\mozilla.cfg", 1)
	Else
		FileInstall("mozilla.cfg", "C:\Program Files\Firefox Nightly\mozilla.cfg", 1)
	EndIf

	FileWrite("C:\Program Files\Firefox Nightly\defaults\pref\local-settings.js", 'pref("general.config.obscure_value", 0);' & @CRLF & 'pref("general.config.filename", "mozilla.cfg");' & @CRLF)
	FileWrite("C:\Program Files\Firefox Nightly\browser\override.ini", '[XRE]' & @CRLF & 'EnableProfileMigrator=false' & @CRLF)
	FileDelete("C:\windows\temp\" & "\FFInstall.exe")
	$CVersion = FileGetVersion('C:\Program Files\Firefox Nightly\firefox.exe', $FV_PRODUCTVERSION)
	SplashOff()
EndFunc   ;==>dlinstall
...
Func EventLog()
	...
	If $ecode = '404' Then
		...
		_EventLog__Report($hEventLog, 1, 0, 404, @UserName, @UserName & ' No "exe" found for Firefox Nightly. The webpage and/or download link might have changed. ' & @CRLF, $aData)
	...
	If $ecode = '411' Then
		...
		$hEventLog = _EventLog__Open("", "Application")
		_EventLog__Report($hEventLog, 0, 0, 411, @UserName, @UserName & " Firefox Nightly " & "version " & $CVersion & " successfully installed." & @CRLF, $aData)
		_EventLog__Close($hEventLog)
	EndIf
